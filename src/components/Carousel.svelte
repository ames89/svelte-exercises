<script lang="ts">
	...
	let { gridClassName, ariaLabelPrev, ariaLabelNext, children } = $props();
	...
	onMount(() => {
		if (trackRef) {
			gap = parseFloat(getComputedStyle(trackRef).gap) || 32;
		}
		updateCarousel();
	...
	const handleNext = () => {
		if (trackRef && viewportRef) {
			...
			if (offset + viewportRef.offsetWidth < track.scrollWidth + (slideWidth + gap)) {
...
<div class="carousel-container">
	<div
		class={`carousel-viewport ${!isScrollable ? 'carousel-centered' : ''}`}
